...
- **Photos**: every `<img>` currently points at a `picsum.photos` random
  ...
  so Vite bundles them, or use any external URL.
...
## Gifts section (Firebase Firestore)
...
others can see and avoid duplicates) needs somewhere to store that shared,
live-updating data, so it uses [Firebase Firestore](https://firebase.google.com/docs/firestore)
...
5. In **Firestore Database → Rules**, replace the default rules with:
   ...
   This lets anyone read the list and add an entry (no login needed, so
   guests don't need an account), but nobody can edit or delete existing
   entries, and each entry is validated to look like a real gift entry.
